...
class VeritabaniYoneticisi:
    ...
    def __init__(self, veritabani_yolu: str = None):
        ...
        if veritabani_yolu is None:
            # Kullanıcının belgeler klasörüne kaydet
            if os.name == 'nt':  # Windows
                belgeler = os.path.join(os.path.expanduser('~'), 'Documents')
            else:  # Linux/Mac
                belgeler = os.path.join(os.path.expanduser('~'), 'Documents')

            uygulama_klasoru = os.path.join(belgeler, 'NotDefteri')
            os.makedirs(uygulama_klasoru, exist_ok=True)
